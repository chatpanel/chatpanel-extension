...
async function voiceRuntime() {
  ...
  if (!voiceLoading) {
    voiceLoading = (async () => {
      const [vc, rules, acted] = await Promise.all([
        import('./js/voice-commands.js'),
        import('./js/rules-builtin.js'),
        import('./js/voice-acted.js'),
      ]);
      ...
      // ONE UTTERANCE, ONE ACTION: the gate holds a request while its words are still moving
      // and the drain acts once they stop. State, so made once here — not per delta.
      const engine = await rules.ruleEngine();
      const gate = vc.createUtteranceGate();
      const drain = vc.createVoiceDrain({
        engine,
        actions,
        gate,
        seen: voiceReported,
        isFresh: async (c, now) => acted.voiceIsFresh(await acted.loadVoiceActed(), c, now),
        remember: acted.rememberVoiceActed,
        isLive: (meetingId) => state.liveMeeting?.id === meetingId,
        onOutcome: (outcome) => {
          const msg = vc.outcomeMessage(outcome);
          if (msg) toast(msg, outcome.ok ? 3000 : 4500);
        },
      });
      voiceMod = { vc, engine, actions, gate, drain };
      return voiceMod;
    })();
  }
  return voiceLoading;
