...
import { webSearchToolProvider as sharedWebSearchToolProvider, searchResultsToText } from './events/web-search-tool.js';
...
// Flatten a webSearch() result into a single readable context blob — SHARED with the
// desktop (`@chatpanel/events/web-search-tool.js`), so both clients hand the model the same
// citation index and the same "no results" wording.
export { searchResultsToText };
...
export function webSearchToolProvider(opts = {}) {
  // The spec, the guidance and the result text are the shared tool; only the search is ours.
  return sharedWebSearchToolProvider({ search: (q) => webSearch(q, opts) });
}
