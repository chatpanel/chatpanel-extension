 ...
 */
export function waveShape({ points = POINTS, t = 0, level = null, height, amplitude = 0.44 } = {}) {
  const maxAmp = Math.max(1, height * amplitude);
  const out = new Float32Array(points);
  for (let i = 0; i < points; i++) {
    // A single travelling sine. The earlier version multiplied two of them, which
    // spends most of its time near zero — the product is why this looked dead.
    const idle = IDLE_BASE + IDLE_SWING * Math.sin(i * 0.38 - t * 0.09);
