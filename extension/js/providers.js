...
// The attachment assembly is shared (events/context-attachments.js): the desktop's turn folds
...
async function streamBridge(agent, messages, { settings, signal, onDelta, onEvent, tools }) {
  ...
  try {
    ...
    throw new Error(
      `Can't reach the ChatPanel Bridge at ${base}. Install ChatPanel (Settings → Agent Tools → ChatPanel local) — the gateway brings the bridge — or start one with \`npx @chatpanel/bridge\`. (${e.message})`,
    );
  }
